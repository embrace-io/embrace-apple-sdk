...
protocol PayloadTest {
    ...
    func evaluate(_ target: String, expecting: String, on: [String: AttributeValue]) -> TestItem
    func testResult(from items: [TestItem]) -> TestResult
}

extension PayloadTest {
    func evaluate(_ target: String, expecting expected: String, on attributes: [String: AttributeValue]) -> TestItem {
        ...
        let result: TestItem.Result = recorded == expected ? .pass : .fail
