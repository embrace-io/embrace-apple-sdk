...
class TerminationReasonParser {
    ...
    static func parse(_ logEntry: String) throws -> TerminationReason {
        ...
        let headerPattern = "<RBSTerminateContext\\|\\s*domain:(\\d+)\\s+code:(0x[A-F0-9]+)\\s+explanation:(.+?)(?=\\nProcessVisibility:|\\n[A-Z])"
        ...
        if let cpuStatsMatch = try matchRegex(pattern: cpuStatsPattern, in: logEntry, options: [.dotMatchesLineSeparators]) {
            
            // Parse total CPU time
            let totalCpuPattern = "Elapsed total CPU time \\(seconds\\): ([\\d.]+) \\(user ([\\d.]+), system ([\\d.]+)\\), (\\d+)% CPU"
        ...
        let thermalPattern = "ThermalInfo:\\s*\\(\\s*\\n\"Thermal Level: (\\d+)\",\\s*\\n\"Thermal State: ([^\"]+)\"\\s*\\n\\)"
        if let thermalMatch = try matchRegex(pattern: thermalPattern, in: logEntry, options: [.dotMatchesLineSeparators]) {
    ...
    private static func matchRegex(pattern: String, in string: String, options: NSRegularExpression.Options = []) throws -> [String]? {
        do {
    ...
    static func formatSummary(_ context: TerminationReason) -> String {
        ...
        lines.append("  Total CPU Time: \(cpu.totalCpuTime?.description ?? "Unknown")s (\(cpu.totalCpuPercent?.description ?? "Unknown")%)")
        lines.append("  User Time: \(cpu.userTime?.description ?? "Unknown")s")
        lines.append("  System Time: \(cpu.systemTime?.description ?? "Unknown")s")
        lines.append("  App CPU Time: \(cpu.appCpuTime?.description ?? "Unknown")s (\(cpu.appCpuPercent?.description ?? "Unknown")%)")
        lines.append("")
