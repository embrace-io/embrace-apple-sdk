...
void proxy_URLSession_task_didSendBodyData(id self, SEL _cmd,
    ...
    id target = [((EMBURLSessionDelegateProxy *)self) getTargetForSelector:_cmd session:session];
    if (target) {
        [target URLSession:session task:task didSendBodyData:bytesSent totalBytesSent:totalBytesSent totalBytesExpectedToSend:totalBytesExpectedToSend];
    }
}
...
void proxy_URLSession_downloadTask_didWriteData(id self, SEL _cmd,
    ...
    id target = [((EMBURLSessionDelegateProxy *)self) getTargetForSelector:_cmd session:session];
    if (target) {
        [target URLSession:session downloadTask:downloadTask didWriteData:bytesWritten totalBytesWritten:totalBytesWritten totalBytesExpectedToWrite:totalBytesExpectedToWrite];
    }
}
...
void proxy_URLSession_downloadTask_didResumeAtOffset(id self, SEL _cmd,
    ...
    id target = [((EMBURLSessionDelegateProxy *)self) getTargetForSelector:_cmd session:session];
    if (target) {
        [target URLSession:session downloadTask:downloadTask didResumeAtOffset:fileOffset expectedTotalBytes:expectedTotalBytes];
    }
}
