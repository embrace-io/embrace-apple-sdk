...
{
    if (sel_isEqual(aSelector, DID_FAIL_NAVIGATION) || sel_isEqual(aSelector, DID_FAIL_PROVISIONAL_NAVIGATION) ||
                    ...
                    decisionHandler:(void (^)(WKNavigationActionPolicy, WKWebpagePreferences *))decisionHandler
{
    ...
    } else if ([self.originalDelegate respondsToSelector:@selector(webView:
                                                             decidePolicyForNavigationAction:decisionHandler:)]) {
        [self.originalDelegate webView:webView
            decidePolicyForNavigationAction:navigationAction
    ...
    shouldAllowDeprecatedTLS:(void (^)(BOOL))decisionHandler API_AVAILABLE(macos(11.0), ios(14.0))
{
    if ([self.originalDelegate
            respondsToSelector:@selector((webView:authenticationChallenge:shouldAllowDeprecatedTLS:))]) {
        [self.originalDelegate webView:webView
