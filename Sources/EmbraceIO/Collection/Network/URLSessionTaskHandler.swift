...
enum CollectorHandlerState {
    case initialized
    case listening
    case paused
}

protocol CollectorHandler {
    func changedState(to collectorState: CollectorState)
}

protocol URLSessionTaskHandler: AnyObject, CollectorHandler {
...
final class DefaultURLSessionTaskHandler: URLSessionTaskHandler {
    @ThreadSafe
    private var state: CollectorHandlerState
    ...
    func changedState(to collectorState: CollectorState) {
        state = collectorState == .listening ? .listening : .paused
    }
}
