...
extension EmbraceTraceMacro: MemberMacro {
    ...
    public static func expansion(
        ...
        guard let structDecl = declaration.as(StructDeclSyntax.self) else {
            context.diagnose(Diagnostic(node: node, message: EmbraceTraceDiagnostic(message: "EmbraceTrace can only be applied to structs")))
        ...
        guard inheritsView else {
            context.diagnose(Diagnostic(node: node, message: EmbraceTraceDiagnostic(message: "Struct must conform to View to use EmbraceTrace")))
        ...
        guard let viewBodyVariable else {
            context.diagnose(Diagnostic(node: node, message: EmbraceTraceDiagnostic(message: "Struct must have a `body` property to use EmbraceTrace")))
